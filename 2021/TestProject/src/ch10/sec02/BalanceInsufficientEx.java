...
public class BalanceInsufficientEx extends Exception{
	...
	//매개변수가 있는 생성자
	//예외 발생 원인(예외 메시지)를 상위 클래스 생성자에게 호출하기 위해 String 타입의 매개변수를 갖는 생성자 사용
	public BalanceInsufficientEx(String message) {
		super(message);
	}
	
	//String 타입의 매개변수를 갖는 생성자
	//-예외 발생 원인(예외 메시지)을 전달하기 위한 생성자
	//-상위 클래스의 생성자들 호출해서 예외 메시지를 전달
	//-예외 메시지의 용도: catch{} 블록에서 예외 처리할 때 
