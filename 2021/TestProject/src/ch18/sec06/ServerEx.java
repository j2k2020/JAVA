...
public class ServerEx {
	...
	//생성자
	public ServerEx() {
		//서버 소캣 생성: 클라이언트 요청 기다림
		try {
			serverSocket = new ServerSocket(9999);
			System.out.println("연결을 기다리고 있습니다.");
			
			//클라이언트로부터 요청이 오면 요청을 수학하고 통신용 소캣 생성
			...
			//통신: 데이터 주고 받음
			//클라이언트에서 'bye' 메시지가 오면 통신 중단
			while(true) {
				//클라이언트로부터 온 메시지를 한 행씩 읽기
				...
				outStream.flush(); //버퍼 비우기(버퍼에 남아 있는 문자열을 모두 강제로 내보냄)
			}
